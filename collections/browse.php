	...
	*/ ?>
	...
	<div class="row" id="grid">
      ...
      <?php if ($item = get_record('Item', array('collection' => metadata($collection, 'id')))): ?>
        <?php echo link_to_items_browse(mdid_thumbnail_tag($item, 'img-responsive') . '<div class="caption"><h5>' . metadata($collection, array('Dublin Core', 'Title')) . '</h5><span class="glyphicon glyphicon-info-sign" aria-hidden="true" data-toggle="popover" data-content="' . metadata($collection, array('Dublin Core', 'Description')) .'"></span></div>', array('collection' => metadata($collection, 'id')), array('class' => 'thumbnail')); ?>
